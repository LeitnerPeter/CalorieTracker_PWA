...
async function renderEntries() {
  ...
  data.forEach(entry => {
    ...
    const li = document.createElement("li");
    li.innerHTML = `
      <span>${entry.name} – ${entry.grams}g – ${entry.calories} kcal</span>
    `;
...
async function addFood(food) {
  ...
  // Chart Cache refresh
  localStorage.removeItem("weeklyCaloriesCache");
...
function getTodayCalories(items) {
  const today = new Date().toISOString().split("T")[0];
...
function renderCalorieStatus(items) {
  const goal = getCalorieGoal();
  const consumed = getTodayCalories(items);
...
function calculateCalories(kcalPer100, grams) {
